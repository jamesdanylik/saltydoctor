...
# video stuff?
# needs install gi see venv creation?
# also needs gst-plugins-bad from Arch repo?
# also needs gst-libav from arch repo?
# obviously livestreamer from pip too

import gi
from gi.repository import Gdk as gdk, GdkX11 as gdkx11, GObject as gobject, Gst as gst, GstPbutils as gstpb
import os
...
class SaltyMainGUI:
	...
	def init_gui(self):
		...
		self.video = tk.Frame(self.master) #, width=640, height=480)
		...
		self.screenshot_btn = tk.Button(self.master, text='Capture', command=self.take_screenshot)
	...
	def take_screenshot(self):
		...
		window = gdkx11.X11Window.foreign_new_for_display(gdk_display, self.video.winfo_id())
		x, y, width, height = window.get_geometry()

		print str(x) + ' ' + str(y) + ' ' + str(width) + ' ' + str(height) 

		# Use 0 offsets to avoid error on window resize?

		pb = gdk.pixbuf_get_from_window(window, 0, 0, width, height)
		print 'pb: ' + str(pb.props.width) + ' ' + str(pb.props.height)

		if pb:
			pb.savev("screenshot.jpeg", "jpeg", (), ())
	...
	def on_source_need_data(self, source, length):
		try:
			...
			exit('Failed to read data from livestream.')
